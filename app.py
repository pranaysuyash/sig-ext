...
uploaded_file = st.file_uploader(
    "Choose an image containing a signature...",
    type=["jpg", "jpeg", "png"]
)
...
if uploaded_file is not None:
    try:
        ...
        if st.session_state.uploaded_image.mode == 'RGBA':
            ...
            background = Image.new("RGB", st.session_state.uploaded_image.size, (255, 255, 255))
            background.paste(st.session_state.uploaded_image, mask=st.session_state.uploaded_image.split()[3])  # 3 is the alpha channel
...
with col1:
    if st.session_state.uploaded_image is not None:
        preview_width = 300
        preview_height = int(preview_width * (st.session_state.uploaded_image.height / st.session_state.uploaded_image.width))
    ...
    if not st.session_state.selection_confirmed:
        with col2:
            ...
            canvas_result = st_canvas(
                ...
                stroke_width=2,
                stroke_color="#ff0000",
                background_image=Image.fromarray(resized_image),  # Use resized_image
            ...
            if canvas_result.json_data is not None:
                ...
                if objects:
                    ...
                    st.session_state.selected_region = (left, top, width, height)

            st.write("2. Choose Signature Color:")
            st.session_state.color = st.color_picker(
                "Pick a color for the signature",
        ...
        with col2:
            ...
            if st.session_state.adjustment_count < 5:
                if st.button('Process Image'):
                    ...
                    color_bgr = tuple(int(st.session_state.color[i:i + 2], 16)
                                      for i in (1, 3, 5))
                    color_mask = np.zeros_like(sig, dtype=np.uint8)
                    for i in range(3):
                        color_mask[:, :, i] = color_bgr[i]

                    sig_color = cv2.addWeighted(
                        sig, 1, color_mask, 0.5, 0)
            ...
            if st.session_state.final_image is not None:
                ...
                final_image_width = min(st.session_state.final_image.width, 600)
                final_image_height = int(final_image_width * (st.session_state.final_image.height / st.session_state.final_image.width))
...
if st.session_state.uploaded_image is not None and st.button("Upload a New Image"):
    reset_session()
    st.rerun()

# --- Gumroad Payment Button ---
st.markdown("---")
st.markdown("###### If you find this tool useful, consider supporting us by making a payment.")
